...
function Dashboard() {
  ...
  const cropImageToSquare = (imageSrc) => {
  ...
  const saveProfilePicture = async (memberType) => {
    try {
      ...
      const croppedBlob = await cropImageToSquare(imageSrc);
      ...
      if (response.data.success) {
        ...
        const profilePicKey = memberType === 'lead' ? 'leadProfilePic' : 
                            memberType === 'member1' ? 'm1ProfilePic' :
                            memberType === 'member2' ? 'm2ProfilePic' : 'm3ProfilePic';
  ...
  const renderProfileImage = (memberType, profilePic, name, borderColor, canEdit) => {
    const isEditing = imageEditMode[memberType];
    const selectedImage = selectedImages[memberType];
    
    return (
      <div className="relative group">
        <div className={`w-20 h-20 rounded-full overflow-hidden border-2 ${borderColor} relative`}>
          {selectedImage ? (
            <img
              src={selectedImage}
              alt={name}
            ...
            <div className={`w-full h-full bg-gray-700 flex items-center justify-center`}>
              <span className={`text-xl font-bold ${borderColor.includes('red') ? 'text-red-400' : 
                                                   borderColor.includes('blue') ? 'text-blue-400' :
                                                   borderColor.includes('yellow') ? 'text-yellow-400' :
                                                   'text-green-400'}`}>
                {name?.charAt(0) || "M"}
              </span>
            </div>
        ...
        <div className="bg-gray-800/70 rounded-xl p-6 mb-6 border border-gray-700 shadow-lg border-yellow-500 border-4">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-4xl font-bold text-yellow-400">TEAM DASHBOARD</h1>
